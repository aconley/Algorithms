...
TEST(GreyCombinationsTest, TestCount1) {
  CountingVisitor vis1;
  combinations::combinations_grey(10, 1, vis1);
  EXPECT_EQ(vis1.getN(), 10) << "10 chose 1 is 10";

  vis1.reset();
  combinations::combinations_grey(20, 1, vis1);
  EXPECT_EQ(vis1.getN(), 20) << "20 chose 1 is 20";
}

TEST(GreyCombinationsTest, TestCount3) {
  CountingVisitor vis3;
  combinations::combinations_grey(7, 3, vis3);
  EXPECT_EQ(vis3.getN(), 35) << "7 chose 3 is 35";
}

TEST(GreyCombinationsTest, TestCount4) {
  CountingVisitor vis4;
  combinations::combinations_grey(10, 4, vis4);
  EXPECT_EQ(vis4.getN(), 210) << "10 chose 4 is 210";

  vis4.reset();
  combinations::combinations_grey(20, 4, vis4);
  EXPECT_EQ(vis4.getN(), 4845) << "20 chose 4 is 4845";
}

TEST(GreyCombinationsTest, TestVaryingT) {
  CountingVisitor vis;
  combinations::combinations_grey(7, 1, vis);
  EXPECT_EQ(vis.getN(), 7);
  vis.reset();
  combinations::combinations_grey(7, 2, vis);
  EXPECT_EQ(vis.getN(), 21);
  vis.reset();
  combinations::combinations_grey(7, 3, vis);
  EXPECT_EQ(vis.getN(), 35);
  vis.reset();
  combinations::combinations_grey(7, 4, vis);
  EXPECT_EQ(vis.getN(), 35);
  vis.reset();
  combinations::combinations_grey(7, 6, vis);
  EXPECT_EQ(vis.getN(), 7);
  vis.reset();
  combinations::combinations_grey(7, 7, vis);
  EXPECT_EQ(vis.getN(), 1);
}

TEST(GreyCombinationsTest, TestCountBig) {
  CountingVisitor vis10;
  combinations::combinations_grey(20, 10, vis10);
  EXPECT_EQ(vis10.getN(), 184756) << "20 chose 10 is 184756";
}
...
TEST(GreyCombinationsBasicTest, Test_6_3) {
  ...
  RecordingVisitor vis;
  combinations::combinations_grey(6, 3, vis);
  EXPECT_EQ(vis.getN(), 20)
    << "Should have 20 visits for 6 objects taken 3 at a time";
   for (int i = 0; i < expected.size(); ++i) {
    EXPECT_EQ(vis.get(i), expected[i])
