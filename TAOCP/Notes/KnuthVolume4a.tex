...
\newitem{208} Independent and maximal subset examples.\hfil\break
...
\noindent [p 358] {\bf Algorithm L} ({\it Lexicographic Combinations})\hfil\break
...
\noindent {\bf Other representations of partitions}

\newitem{349} Conjugate partitions.  Why does the relation $b_j - b_{j+1} = c_j$
...
\newitem{395} Why are there $n$ inversions in the rim representation?  Well, first,
...
\newitem{396} Upper bound on the number of partitions.
...
\newitem{400} Theorem E \hfil\break
...
\noindent [p 444] {\bf Trees as Inversion Tables} \hfil\break

The fact that $c_1 = 0$ is just a standard fact of inversion tables
(at least in this order), but the requirement that $c_{k+1} \le c_k + 1$
is not true in general for all inversion tables.  For example,
consider the permutation $4231$, which has the inversion table $0113$
so that $c_4 > c_3 + 1$.

So why is it true then?  Well, each $c_k$ is essentially how deeply nested
you are at each left parenthesis (as the text notes), and you can't increase
that depth by more than one in one place -- although you can decrease it.
