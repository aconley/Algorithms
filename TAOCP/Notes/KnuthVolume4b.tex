...
\noindent [p 3] Formula (12): Expectation of expectations: show that
...
Looking at this in terms of events also tells you that the formula must be extendable
to include additional conditioning:
$$
 E\left( E\left(X | A_1, \ldots, A_n, B \right) | A_1, \ldots, A_n \right) 
  = E \left(X | A_1, \ldots, A_n \right)
$$
since conditioning over $B$ is just the same thing as restricting the probablity space.
Note that it's the variables that are not conditioned over that disappear in the expectation.
The same holds for random variables: $E\left( E\left(X | Y_1, \ldots, Y_n, Z\right) |
Y_1, \ldots, Y_n \right) = E\left(X | Y_1, \ldots, Y_n \right)$.

Anyways, returning to the formula: as explained in the text, $E\left(X|Y\right)$
...
\phantom{2}\hskip1em If the iterator state is READY, set 
 $l \leftarrow l - 1.$\hfil\break
\phantom{3}\hskip1em If the iterator state is NEW, set $l \leftarrow 0$, 
 compute $S_0$, and set the state to READY.\hfil\break
\noindent {\bf WI2} Repeat this step while $S_l = \emptyset$: if $l = 0$ 
 then set the state to DONE and terminate the iterator.  Otherwise, 
 $l \leftarrow l - 1$.\hfil\break
\noindent {\bf WI3} Set $x_l \leftarrow {\rm min}\, S_l$, 
 $S_l \leftarrow S_l / x_l$, and $l \leftarrow l + 1$.\hfil\break
\noindent {\bf WI4} If $l = n$, then return 
 $\left(x_0, \ldots, x_{n-1}\right)$.\hfil\break
...
sentinel value $s_l$ at each level which is not part of ${\cal D}_l$, and which
is more than any valid value and which is the sucessor of the maximum valid 
${\cal D}_l$.  For example, for Sudoku we could use 10 as a maximum sentinel, 
for Langford pairs or for $n$-queens we could use $n+1$, etc.  Rather than using
a maximum sentinel value, one can use a minimum instead.
...
\phantom{2}\hskip1em If the iterator state is READY, set 
 $l \leftarrow l - 1.$\hfil\break
\phantom{3}\hskip1em If the iterator state is NEW, set $l \leftarrow 0$, set
the state to READY, and initialize any datastructures.\hfil\break
\noindent {\bf BI2} If $x_l = s_l$ then: if $l = 0$, set the state to DONE and 
  terminate the iterator.  Otherwise, backtrack by setting $l \leftarrow l - 1$, 
  downdate the datastructures appropriately, set 
  $x_l \leftarrow {\rm next}\,{\cal D}_l$, and repeat this step.\hfil\break
\noindent {\bf BI3} If $P\left(x_0, \ldots, x_l\right)$ then set 
  $l \leftarrow l + 1$.  If $l = n$ than return 
  $\left(x_0, \ldots, x_{n-1}\right)$.  Otherwise update the datastructures for 
  the new level, set $x_l \leftarrow {\rm min} {\cal D}_l$, and return to 
  {\bf BI2}.\hfil\break
\noindent {\bf BI4} Set $x_l \leftarrow {\rm next}\,{\cal D}_l$ and return 
 to {\bf BI2}.
\vskip 0.1in
...
\vskip 0.1in
\noindent {\bf WS1} Set $l \leftarrow 0$ and compute $S_0$.\hfil\break
\noindent {\bf WS2} Repeat while $S_l = \emptyset$: if $l = 0$ report failure
  (no solution was found).  Otherwise, $l \leftarrow l - 1$.\hfil\break
\noindent {\bf WS3} Set $x_l {\rm min} S_l$, $S_l \leftarrow S_l / x_l$, 
and $l \leftarrow l + 1$.\hfil\break
\noindent {\bf WS4} If $l = n$ then return the solution 
  $\left(x_0, \ldots, x_{n-1}\right)$. \hfil\break
\noindent {\bf WS5} Compute $S_l$ and return to {\bf WS2}.
\vskip 0.1in

\noindent and for the vanilla backtrack:

\vskip 0.1in
\noindent {\bf BS1} Set $l \leftarrow 0$ and initialize the 
 datastructures.\hfil\break
\noindent {\bf BS2} If $l = n$, return the solution.  Otherwise, set 
 $x_l \leftarrow {\min}\,{\cal D}_l$.\hfil\break
\noindent {\bf BS3} If $P_l\left(x_0, \ldots, x_{l-1}\right)$ holds, update the 
 data structures to facilitate testing $P_{l+1}$, set $l \leftarrow l + 1$, and 
  go to {\bf BS2}.\hfil\break
\noindent {\bf BS4} If $x_l \neq {\rm max}\,{\cal D}_l$, set 
  $x_l \leftarrow {\rm next}\,{\cal D}_l$ and go to {\bf BS3}.\hfil\break
\noindent {\bf BS5} While $x_l = {\rm max}\,{\cal D}_l$ repeat: if $l = 0$ 
 report failure, otherwise set $l \leftarrow l - 1$ and downdate the data 
 structure changes made in {\bf BS3}.\hfil\break
\noindent {\bf BS6} $x_l \leftarrow {\rm next}\,x_l$ and return to 
 {\bf BS3}.\hfil\break
