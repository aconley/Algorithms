...
We can do something similar for the non-Walkers approach  and with the same 
...
\sec {Section 7.2.2.1: Dancing Links}

\algorithm {69} {Algorithm X}
...
One could instead have a lazy version of Algorithm~X that doesn't remove the
...
\subsec {Secondary Items}

\newitem {72} Removing symmetry with secondary items

The way this is presented is a bit confusing -- after all, the example actually
...
\subsec {Factoring an Exact Cover Problem}

\newitem {83} An exact cover problem solves other problems.
\vskip 0.1in
...
\newitem {84} Cover a chessboard with 21 straight trominoes and one monomino.
\vskip 0.1in
...
\newitem {86} We know that five of the pieces 1, 2, 4, 5, 6, 7 occupy as
...
\subsec {Analysis of Algorithm~X}

\newitem {98} Strictness
...
\newitem {99} Nodes in the search tree
...
\subsec {Maintaining a decent focus}

\newitem {105} Direct sums of search trees: the direct sum ($A \oplus A^{\prime}$)
...
\subsec{Exploiting local equivalence}

\newitem {107} Why does the new tree in (100) solve the focus problem?
