...
\topglue 0.5in
\centerline{\tt Knuth 7: Combinatorical Searching}
\vskip 0.3in
...
\vskip 0.1in
\noindent
{\bf Problem 42} {\it Let G be a graph with 13 vertices in which every vertexhas degree 5.
Make a non-trivial statement about G}\hfil\break
It doesn't exist.  Recall that the sum of the degrees of all vertices must be
twice the number of edges (since it counts each edge twice, and we aren't allowing
self-loops).  Therefore, the number of edges is $65/2$, which isn't possible.
...
Incidentally, if one doesn't want to start with projection and negation, those
can be constructed from NAND.  First, $\bar x = x \bar \land x$.  Then we
can form $1 = x \bar \land \bar x = x \bar \land \left( x \bar \land x \right)$ 
and use that to form $L = \overline{ x \bar \land 1}$ (writing $0$ in terms of $y$), and
$R = \overline{ y \bar \land 1}$.  Also: $0 = \overline{\left( x \bar \land \bar x \right)}.$

\newprob {10} Find the multilinear representation of $1100 \, 1001 \, 0000 \, 1111$.

\noindent This is a bit tedious.  We want to break down the above truth table using
the decomposition $f\left(x_1, \ldots, x_n\right) = g\left(x_1, \ldots, x_{n-1}\right) 
\oplus h\left(x_1, \ldots, x_{n-1}\right) \wedge x_n$, where 
$g\left(x_1, \ldots, x_{n-1}\right) = f\left(x_1, \ldots, x_{n-1}, 0 \right)$ and
$h\left(x_1, \ldots, x_{n-1}\right) = f\left(x_1, \ldots, x_{n-1}, 0 \right) 
\oplus f\left(x_1, \ldots, x_{n-1}, 1\right)$ of (16).  To be a bit more concrete,
if the truth table of $f$ is $abcd efgh$, then $g = aceg$ and $h = aceg \oplus bdfh$.

Labeling the variables $wxyz$, for the first step $g = 1010\,0011$, 
$h = 1010\,0011 \oplus 1001\,0011 = 0011\,0000$,
and so we have $f = 1010\, 0011 \oplus 0011 \, 0000 \wedge z$.
The first term is ($1010\,0011$), so we next have $g = 1101$, 
$h = 1101 \oplus 0001 = 1100$, so the first term is equivalent to
$1101 \oplus 1100 \wedge y$.  The second term ($0011\, 0000$) has
$g = 0100$, $h = 0100 \oplus 0100 = 0$, which becomes $0100 \oplus 0 \wedge y = 0100$.
Thus, after the first round, we have $f = \left(1101 \oplus 1100 \wedge y\right) 
\oplus \left(0100\right) \wedge z$.

Continuing with the $f = \left(1101 \oplus 1100 \wedge y\right)$ term,
for $1101$ we have $g = 10$, $h = 10 \oplus 11 = 01$, so this is $10 \oplus 01 \wedge x$,
while for $1100$ we have $g = 10$, $h = 10 \oplus 10 = 0$, and so this is $10 \oplus 0 \wedge x = 10$.
Substituting, we now have
$$
f = \left( \left(10 \oplus 01 \wedge x\right) \oplus 10 \wedge y \right) \oplus 0100 \wedge z.
$$
Continuing with $10$ gives $g= 1$, $h = 1 \oplus 0 = 1$, or $1 \oplus 1 \wedge w = 1 \oplus w$,
and $01$ is $g = 0$, $h = 1$, or $0 \oplus 1 \wedge w = w$.  Therefore,
$01 \oplus 01 \wedge x = \left(1 \oplus w\right) \oplus w \wedge x$.  Substituting, we now have
$$
f = \left( \left(1 \oplus w \right) \oplus w \wedge x \oplus \left(1 \oplus w\right) \wedge y \right)
 \oplus 0100 \wedge z.
$$

Now we want to explore $0100 \wedge z$. Next, $0100$ has $g = 0$, $h=10$, so this is
$\left( 00 \oplus 10 \wedge x\right) \wedge z$.  We already know that $10$ is $1 \oplus w$,
so we finally have $\left(1 + w\right) \wedge x \wedge z$, and finally putting it all together
$$
 \eqalign{
   1100 \, 1001 \, 0000 \, 1111 &= 
      \left( \left(1 \oplus w \right) \oplus w \wedge x \oplus \left(1 \oplus w\right) \wedge y \right)
      \oplus \left(1 + w\right) \wedge x \wedge z \cr
    &= 1 + w + w x + y + wy + xz + wxz . \cr
 }
$$

\newprob {12} ({\it multilinear representation}).\hfil\break
...
\newprob {21} Prove that a nonconstant Boolean function is monotone if and only if
it can be expressed entirely in terms of the operations $\wedge$ and $\vee$.

\noindent Recall that a monotonic boolean function is a function such that
it's value does not change from 1 to 0 whenever any of it's variables changes
from 0 to 1.  First, any expression formed purely in terms of $\wedge$ and $\vee$
that does not have any complemented variables is clearly monotonic, because
the component operations are monotonic.  

Second, consider Boole's decomposition law (the law of development):
$$
f\left(x_1,\ldots,x_n\right) = 
  \left(g\left(x_1, \ldots, x_{n-1}\right) \wedge {\bar x}_n \right) \vee
  \left(h\left(x_1, \ldots, x_{n-1}\right) \wedge x_n\right),
$$
where $g\left(x_1, \ldots, x_{n-1}\right) = f\left(x_1, \ldots, x_{n-1}, 0\right)$
and $h\left(x_1, \ldots, x_{n-1}\right) = f\left(x_1, \ldots, x_{n-1}, 1\right)$.

From this we can see that $g$ and $h$ must also be monotonic, and that $g \le h$ 
for all inputs.  Why?  First, consider what would happen if $g$ was not monotonic.
Then there would be some value $x_k$ such that changing $x_k$ from 0 to 1 would change
$g$ from 1 to 0.  If this were the case, then setting $x_n = 0$ and flipping $x_k$
from 1 to 0 would also change $f$ from 1 to 0, so then $f$ could not be monotonic.
A similar argument applies to $h$, but with $x_n = 1$.

For $g \le h$, if this were not the case, then there would be some set of inputs such
that $g\left(x^{\prime}_1, \ldots, x^{\prime}_{n-1}\right) >
h\left(x^{\prime}_1, \ldots, x^{\prime}_{n-1}\right)$.  But if this were the case, then
$f\left(x^{\prime}_1, \ldots, x^{\prime}_{n-1}, 0\right) >
f\left(x^{\prime}_1, \ldots, x^{\prime}_{n-1}, 1\right)$, which violates monotonicity.

This implies that
$$
 \eqalign{
    f &= \left(g \wedge {\bar x}_n \right) \vee \left(h \wedge x_n\right) \cr
      &= \left(g \wedge {\bar x}_n \right) \vee \left(g \wedge x_n \right) \vee \left(h \wedge x_n\right) \cr
      &= g \vee \left(h \wedge x_n\right) .\cr
 }
$$
where the second line follows because $g \le h$ and so the addition of the extra term can't
change the value of $f$ for any arguments, and the final line follows by logical resolution.
Therefore, if we decompose the truth table for a monotonic function, we never need to introduce
complementary variables.  Furthermore, constants will only appear if the function is constant,
...
\topglue 0.5in
\centerline{\tt Knuth 7.1.2: Boolean Evaluation}
\vskip 0.3in
...
\topglue 0.5in
\centerline{\tt Knuth 7.1.3: Bitwise Tricks and Techniques}
\vskip 0.3in
...
\vskip 0.08in \noindent {\bf Problem 9} ({\it Nim}) \hfil\break
...
\topglue 0.5in
\centerline{\tt Knuth 7.1.4: Binary Decision Diagrams}
\vskip 0.3in

\noindent {\bf Problem 1} Done -- and definitely worth doing.  But getting
...
\vskip 0.08in \noindent {\bf Problem 3} There are 2 with size 1 (the constants truth and 
...
\vskip 0.08in \noindent {\bf Problem 25} ({\it Compute the generating function})
...
\vskip 0.08in \noindent {\bf Problem 26} ({\it Compute the reliability polynomial})
...
\vskip 0.08in \noindent {\bf Problem 30} Consider diagram 26.
...
\vskip 0.08in \noindent {\bf Problem 35} We want to walk up the BDD,
...
\vskip 0.08in \noindent {\bf Problem 53} ({\it Reduction to BDD})\hfil \break
...
If the nodes are labeled $a, b, c, d, e, f, g$ in tree order (so $a$ is the
...
The final step is $p \gets d$, $s \gets \sim {\tt AUX}\left(d\right) = 0$,
...
R4 does $s \gets e$ and ${\tt AUX}\left(0\right) \gets 0$,
...
\vskip 0.08in \noindent {\bf Problem 54} There are a couple options.  A sensible method
...
\vskip 0.08in \noindent {\bf Problem 186} This is a function where all variables have to be
...
\vskip 0.08in \noindent {\bf Problem 187} Also instructive -- but also getting
...
\vskip 0.08in \noindent {\bf Problem 208} ({\it Modify Algorithm C to work with ZDDs})
...
\vskip 0.08in \noindent {\bf Problem 213} A classic interview question.  This
...
\topglue 0.5in
\centerline{\tt Knuth 7.2.1.1: N-tuples}
\vskip 0.3in

\noindent {\bf Problem 1} When initializing, set $l_0 \gets 0$
and $u_0 \gets 1$, and in M4 change the test from $a_j = m_j - 1$
to $a_j = u_j$, and $a_j \gets 0$ to $a_j \gets l_j$.

\vskip 0.08in \noindent {\bf Problem 6} Let's say that the relation has been proved for
...
\vskip 0.08in \noindent {\bf Problem 9} The position is 1011000.
$b_0 = a_0 \oplus a_1 \ldots = 1$ (the number of 1s is odd),
so $k$ is odd and we want to put the last ring on.

\topglue 0.5in
\centerline{\tt Knuth 7.2.1.2: Permutations}
\vskip 0.3in

\newprob {1} Make Algorithm~L faster in the case where $j \sim n$.\hfil\break
...
$a_{n-2}$:
\newstep{L2.1} Set $b \gets a_{n-1}$, $c \gets a_{n}$.  If $b < c$, set
$a_{n-1} \gets c$, $a_n \gets b$ and return to {\bf L~1}.
\newstep{L2.2} Set $j \gets n - 2$ and $c \gets a_j$.  While $c \ge b$, set
$j \gets j - 1$, $b \gets c$, $c \gets a_j$.  Terminate if $j = 0$.
We can also expand steps 3 and 4 by adding a special case step:
\newstep{L3.1} If $c < a_n$, set $a_j \gets a_n$, $a_{j+1} \gets c$, $a_n \gets b$
and go to {\bf L4.2}.
\newstep{L3.2} Set $l \gets n - 1$.  If $c \ge a_l$, decrease $l$ by 1 until $c < a_l$,
and set $a_j \gets a_l$, $a_l \gets c$.
\newstep{L4.1} Set $a_n \gets a_{j+1}$ and $a_{j+1} \gets b$.
\newstep{L4.2} Set $k \gets j + 2$ and $l \gets n - 1$.  While $k < l$,
interchange $a_k$ and $a_l$ and set $k \gets k + 1$, $l \gets l - 1$.  Return to {\bf L1}.

\newprob{3} Determine the rank of a permutation $a_1 a_2 \ldots a_n$ in
Algorithm~L.\hfil\break
The important thing to understand is the way that Algorithm~L changes the inversion
table.  For $n=3$, the sequence $123, 132, 213, 231, 312, 321$ has the inversion
table $000, 010, 100, 110, 200, 210$.  That is, it appears to be counting in the mixed
radix number system $b = \left(n, n-1, n-2, \ldots, 1\right)$ -- that is, the factorial
number system.  And inspection of Algorithm~L shows that is exactly what it is doing --
finding the $j$ to increase is finding the rightmost digit that can be increased, and then
putting the remaining elements in order is setting all the subsequent digits to zero.
Therefore, the rank of $a_1 \ldots a_n$ is:
\newstep{R1} Form the inversion table for $a$: $c_1 \ldots c_n$.  This can be done
efficiently by a merge sort-like procedure if $n$ is large.
\newstep{R2} The rank is the mixed radix number
$$
 {c_1\,c_2\,\ldots\,c_{n-1}\,c_n \brack n\,n-1\,\ldots\,2\,1} .
$$
Therefore, the rank of 314592687 is ${2\,0\,1\,1\,4\,0\,0\,1\,0 \brack 9\,8\,7\,6\,5\,4\,3\,2\,1} =
81577$.

\newprob{8} Modify Algorithm~L to visit all variations of a multiset.\hfil\break
For each permutation we would visit in {\bf L1}, simply visit
each prefix of that permutation.  For example, for 1224, visit $\epsilon$, 1, 12, 122, and 1224
in ${\bf L1}$.  However, note that this will visit many of the variations multiple times.

\newprob{21} True or false: if an alphametic has a solution in radix $b$,
it has a solution in radix $b + 1$.\hfill\break
It suffices to find a single counterexample: ${\tt A} + {\tt A} = {\tt AB}$ only
has a solution for $b = 2$ (with {\tt A} = 1, {\tt B} = 0).

\newprob{25} How can you quickly compute $\rm{min}\left(a \cdot s\right)$
and $\rm{ max}\left(a \cdot s\right)$?\hfil\break
We have the freedom to chose the letter codes to put $s$ in order (e.g., $s_1 \le s_2 \le \ldots
\le s_10$).  Then simply iterate through the elements of $a \notin F$ in that order.  So, if
the least index is $j$ , then $a_j = 0$, etc.  The max is the same procedure with the signs of
the signatures flipped.

\topglue 0.5in
\centerline{\tt Knuth 7.2.1.3: Combinations}
\vskip 0.3in

\newprob{5} Solutions to integer inequalities.

\noindent What is a good way to generate all of the integer solutions to the following
systems of inequalities?

\vskip 0.05in
\noindent a) $n > x_t \geq x_{t-1} > x_{t-2} \geq x_{t-3} > \cdots > x_1 \geq 0$
when $t$ is odd?

\vskip 0.04in
\noindent Recall that a $\left(s, t\right)$ combination is a solution to the inequalities
$n > c_t > \cdots > c_2 > c_1 \geq 0$.  Also note the alternating $\geq, >$.  This suggests we 
take some form of $\left(s, t\right)$ combination and subtract either one or zero
in an alternating fashion, e.g. some form of $k /\!/ 2$, where $/\!/$ is integer division
(alternatively, $\lfloor k / 2 \rfloor$).  More precisely, to get it to line up, we need 
$\left(k - 1\right) /\!/ 2$.  We have collapsed $t /\!/ 2$ of them, so: form 
${n + t /\!/ 2 \choose t}$ and subtract $\left(k - 1\right) /\!/ 2$ from each $c_k$.

\vskip 0.08in
\noindent b) $n \gg x_t \gg x_{t-1} \gg \cdots \gg x_2 \gg x_1 \gg 0$ where
$a \gg b$ means $a \geq b + 2$.

\vskip 0.04in
\noindent Basically the same idea, but now we are adding: $c_k \leftarrow c_k + k + 1$
and we only need ${n - t - 2 \choose t}$ of them.

\newprob{7} Design an algorithm that runs through the dual combinations $b$
in dexreasing lexical order (as in table~1).

\vskip 0.05in
\noindent Recall that $n = s + t$.  I will use a 0 basis -- e.g., $b_{s-1}, b_{s-2}, \ldots, b_0$.

\vskip 0.05in
\noindent Here $k$ will be the index of the value we are currently modifying.
\vskip 0.05in
\noindent 
{\bf B1} [Initialize] For $k$ in $\left[s - 1, 0\right]$ set $b_k = k + t$.\hfil\break
{\bf B2} [Visit] Visit $b_{s-1}, \ldots, b_0$.  If $k = s$, terminate. \hfil\break
{\bf B3} [Decrease $b_k$] Set $b_k \leftarrow b_k - 1$. \hfil\break
{\bf B4} [Increase $k$] If $b_k = k$, set $k \leftarrow k + 1$ and go to {\bf B2}.\hfil\break
{\bf B5} [Fix up $b_{k-1}, \ldots, b_0$] While $k > 0$ set $b_{k-1} \leftarrow b_k - 1$.
  Goto {\bf B2}.\hfil\break

\newprob{8} Design an algorithm that generates all $\left(s, t\right)$ combinations
$a_{n-1}, \ldots, a_0$ lexicographically in bitstring form.

\vskip 0.05in\noindent Here $i$ will be the index of the rightmost 0 and $j$ the index
of the rightmost 1.
\vskip 0.05in
\noindent
{\bf A1} [Initialize] Set $a \leftarrow 0^s 1^t$, $i \leftarrow t$, $j \leftarrow 0$.\hfil\break
{\bf A2} [Visit] Visit $a_{n-1}, \ldots, a_0$.  If $i = 0$ ($a$ ends in a 0) goto 
  {\bf A4}. \hfil\break
{\bf A3} [Easy case: change $01^i \rightarrow 101^{i-1}$] Set $a_i \leftarrow 1$, 
  $a_{i-1} \leftarrow 0$, $i \leftarrow i - 1$.  If $i = 0$ set $j \leftarrow 1$.
  Goto {\bf A2}.\hfil\break
{\bf A4} [Hard case: $\ldots 01^q0^{j-1} \rightarrow \ldots 0^{j}1^{q-1}$]
 If $j = n-1$ terminate.  Otherwise set $a_j \leftarrow 0$, $j \leftarrow j + 1$.
 If $a_j = 1$, set $a_i \leftarrow 1$, $i \leftarrow i + 1$ and goto {\bf A4}.\hfil\break
{\bf A5} [Fix leading 1] Set $a_i \leftarrow 1$.\hfil\break
{\bf A6} [Fix $j$] If $i > 0$ set $j \leftarrow 0$.  Goto {\bf A2}\hfil\break

\vskip 0.05in
\noindent {\bf A4} does things like $\ldots 01110 \rightarrow 00011$.  Note that the
leading 1 is absent, hence {\bf A5}.  It also re-uses $j$ to index it's position, so 
that within this step $j$ may not be the position of the rightmost 1, which is why {\bf A6} is needed.


\topglue 0.5in
\centerline{\tt Knuth 7.2.1.4: Partitions}
\vskip 0.3in

\newprob{1} The twelvefold way.

\noindent Note that the ordering of the balls within each urn is not considered,
even if they are labeled.

\noindent 1: $n$-tuples of $m$ things: there are $m$ ways to chose each value,
and hence $m^n$ total possibilities.

\vskip 0.05in
\noindent 2: $n$ permutations of $m$ things:  There are $n$ choices for the first
element, $n-1$ for the second, $n-2$ for the third, etc.  Therefore there are
$n \times \left(n - 1\right) \times \ldots \left(n - m\right) = m^{\underline{n}}$
combinations.

\vskip 0.05in
\noindent 3: partitions of ${1, \ldots, n}$ into $m$ ordered parts.  These are
set partitions, so there are ${n \brace m}$ of them.  However, Stirling numbers ignore
ordering of the sets.  Since there are $m$ sets, we must correct by multiplying
by the number of arrangements, giving $m! {n \brace m}$ possibilities.

\vskip 0.05in
\noindent 4: $n$-multicombinations of m things: see 1.2.6-60, or alternatively 7.2.1.3-(6).
So we have $s + 1 = m$ and $t = n$, so ${n + m - 1 \choose n}$ possibilities.

\vskip 0.05in
\noindent 5: $n$-combination of $m$ things is ${m \choose n}$.

\vskip 0.05in
\noindent 6: composition of $n$ into $m$ parts: see 7.2.1.3-(9), so ${n - 1 \choose n - m} =
{n - 1 \choose m - 1}$.

\vskip 0.05in
\noindent 7: partitions of ${1, \ldots, n}$ into $\leq m$ parts.  Recall that the
partition of $n$ into exactly $m$ parts is ${n \brace m}$, so this is
${n \brace 1} + \ldots {n \brace m}$.

\vskip 0.05in
\noindent 8: $n$ pigeons into $m$ holes: they either fit (if $n \leq m$) or they don't,
but we can't tell the difference between any ways of fitting them, so this is just
the indicator function $\left[n \leq m\right].$

\vskip 0.05in
\noindent 9: partitions of ${1, \ldots, n}$ into $m$ parts is ${n \brace m}$.

\vskip 0.05in
\noindent 10: partitions of $n$ into $\leq m$ parts.  Recall that the partition into
exactly $m$ parts is $\vert {n \atop m} \vert$, so $\vert {n \atop 1} \vert + \ldots
+ \vert {n \atop m} \vert.$

\vskip 0.05in
\noindent 11: $n$ pigeons into $m$ holes: again $\left[ n \leq m \right]$.

\vskip 0.05in
\noindent 12: partitions of $n$ into $m$ parts: $\vert {n \atop m} \vert$.
