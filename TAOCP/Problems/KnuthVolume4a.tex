...
\newprob{Problem 1} Make Algorithm~L faster in the case where $j \sim n$.\hfil\break
...
\newprob{Problem 3} Determine the rank of a permutation $a_1 a_2 \ldots a_n$ in
...
\newprob{Problem~8} Modify Algorithm~L to visit all variations of a multiset.\hfil\break
...
\newprob{Problem~21} True or false: if an alphametic has a solution in radix $b$,
...
\newprob{Problem 25} How can you quickly compute $\rm{min}\left(a \cdot s\right)$
...
\newprob{Problem 5} Solutions to integer inequalities.
...
\newprob{Problem 7} Design an algorithm that runs through the dual combinations $b$
...
\newprob{Problem 8} Design an algorithm that generates all $\left(s, t\right)$ combinations
...
\newprob{Problem 1} The twelvefold way.
