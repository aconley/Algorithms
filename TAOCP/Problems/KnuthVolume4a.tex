...
Knuth's answer gives a branch free version: $u \gets y \, \& \, \left(y + 1\right)$, 
...
is $m$ $01$s, which is $\left(2^{2m}-1\right)/3$. \hfil\break
b) The general rule for finding the successor of any lexicographic pattern $a_1 \ldots a_n$
is to find the largest $j$ such that $a_j$ can be increased, increase
...
The key here is that $\ldots010101$ is the largest possible value.  So we want
to find the rightmost element that is out of position with respect to that.
We can extract that bit by doing $z \gets x \oplus \mu_0$, $z \, \& \, -z$,
where $\mu_0 = -1/3 = \ldots0101$.  Alternatively, we can extract and smear
the right  with $z \oplus \left(z - 1\right)$.  If we add one to the latter, we'll
get the position before the last out of place element... but that isn't quite what
we want, because there might be 1s before that.  So, really, we want the
set of 1s containing the rightmost out of place value -- note that the rightmost
...
So if $u = z \oplus \left(z - 1\right)$, $v = x\, |\, u$, $w = v + 1$
we get the 0 just before the out of place run increased by one.
So, this is the increasing the rightmost $a_j$ by the smallest amount.
...
1 bit in $w$?  We need $\rho w + \nu w - n /2$ 0s in a $n$-bit word,
which, according to 7.1.3.61 is equal to $1 + \nu v - n / 2$.  That
may actually not be a bad way to approach it on MMIX, but it
would be preferable to have something that doesn't depend on $n$.
Recall that $u$ is a mask for the rightmost element out of place
and all points to the right of it.  That's what we need to fill half with 0s
and half with 1s.  So we compute $v \, \& \, {\bar w} \gg \left(\nu u \gg 1\right)$,
or, alternatively $v \, \& \, \bar{w} \gg \left(\nu \left(u \, \& \, \bar \mu_0\right)\right)$, or
$v \, \& \, {\bar w} / \sqrt{u + 1}$, and add that to $w$.

So, putting it together: $z = x \oplus \mu_0$, $u = z \oplus \left(z - 1 \right)$,
$v = x\, |\, u$, $w = v + 1$, $y = w + \left(v \, \& \, {\bar w}\right) / \sqrt{u + 1}$.
To actually implement this, you wouldn't want to do the division.
