...
However, what we really want is the other way -- if $s \ne 0$,
can we force $t = 0$?  Sure!  $s$ must have a non-zero bit --
let's say the leftmost one is in position $d$.  Now chose $j$
so that the dth bit of $x_j$ is also zero.  There must be at
least one such pile, or $s$ would have a zero there.
Now subtract such that $y_j = s \, \oplus \, x_j$. This
is less than $x_j$ since all the bits to the left are the same
but the dth changes from 1 to 0.  So the move is to remove
$x_j - y_j$ sticks from $j$, and we have $t = s \, \oplus \, x_j
\, \oplus \, s \, \oplus \, x_j = 0$.  Note this is a rephrasing
of bits of the proof of the Sprague-Grundy theorem.

The connection to mex is a bit mysterious here, however.  If $x$ is
the size of the pile, then what is $S$?  It is $S = \{ 0, 1, \ldots, x - 1 \}$.
These are all the legal states that the pile can be in after the player moves,
since they are forced to remove at least one stick but can remove up to
$x$ of them.  Now, if we also have a pile $y$ corresponding to the set
$T = \{0, 1, \ldots, y - 1\}$, then what are all the possible states
of the game after the player moves?  Well, if the player takes sticks
from pile $x$ then it is some element of $S$ plus $y$ in the other pile,
and if the player takes from $y$ it is $x$ plus some elements of $T$.
This corresponds to the two cases in the Sprague-Grundy theorem --
$S \, \oplus \, y$ and $x \, \oplus \, T$, respectively, although the
use of $\oplus$ still appears rather strange.
\end