...
import sedgewick.search.UnorderedTable

class UnorderedTableTest extends FlatSpec with Matchers {
  val example = "ABCDEF".toList.zipWithIndex
  val tbl = UnorderedTable(example:_*)

  "An UnorderedTable" should "have the right number of elements" in {
  ...
  "it" should "collapse duplicates on construction" in {
    val tbl2 = UnorderedTable("SEARCHSEARCHAC".toList.zipWithIndex:_*)
