...
import sedgewick.search.OrderedTable

class OrderedTableTest extends FlatSpec with Matchers {
  val example = "ABCDEF".toList.zipWithIndex
  val tbl = OrderedTable(example:_*)

  "An OrderedTable" should "have the right number of elements" in {
  ...
  "it" should "collapse duplicates on construction" in {
    val tbl2 = OrderedTable("SEARCHSEARCHAC".toList.zipWithIndex:_*)
