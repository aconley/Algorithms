...
mod tests {
    ...
    /// Marks each `(from, to)` pair as a true arc variable, leaving every
    /// other variable at its default (`TernaryVal::DontCare`, which
    /// `from_model` treats as false).
    fn model_of(graph: &UnGraph<(), ()>, arcs: &[(usize, usize)]) -> Assignment {
    ...
    /// The 13-vertex, 21-edge graph from `.agents/algorithm.md`'s worked
    /// example (also built independently in `encoding.rs`'s tests).
    ///
    /// Node indices follow the book's A=0 .. M=12 mapping.  This helper and
    /// the two below it are meant to be reused by phase 9's merge tests, per
    /// `plan.md`.
    fn knuth_graph() -> UnGraph<(), ()> {
        graph_of(
            13,
            &[
                (0, 1),
    ...
    fn knuth_cover_arcs() -> Vec<(usize, usize)> {
        vec![
            (0, 2),
            (2, 6),
            (6, 3),
