...
enum BasicBacktracking {
 ...
 impl BasicBacktrackingData {
    ...
    fn threshold(&self) -> usize {
     ...
     pub fn remove_false_literal(&mut self, lit: u32) -> bool {
...
+            if self.size[j] == 1 {
    ...
    /// occurrence lists, decrements occurrence counts, and reduces `a` by `c[lit]`.
     pub fn deactivate_literal(&mut self, lit: u32) {
        let threshold = self.threshold();
     ...
     /// Does not modify `d`.
     pub fn reactivate_literal(&mut self, lit: u32) {
        let threshold = self.threshold();
         let lit_idx = lit as usize;
     ...
     pub fn unremove_false_literal(&mut self, lit: u32) {
         let not_lit = (lit ^ 1) as usize;
        let threshold = self.threshold();
