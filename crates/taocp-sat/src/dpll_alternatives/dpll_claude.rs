...
mod tests {
    use super::*;
    use crate::{sample_problems::waerden, Clause};
    ...
    fn test_is_unit_with_false_other_lit() {
        ...
        // The clause watches whichever lit is at start[1]; with rev order, larger lit first
        // Clause [2,4] stored as [4,2] (rev sorted), so l[start[1]]=4 watched
        // is_unit(4,...) should return true since x2 is set true (move 1 = x=0, so pos is false? wait)
        // move 1 = x=0: (lit + move) & 1: for lit=4 (pos x2): (4+1)&1=1 → false. yes.
        // is_unit(4, ...) checks watch list of lit 4; clause 1 has l[start[1]]=4 watched.
        // p=start+1, l[p]=2, is_unit scans: literal_is_false(2,...) → depth_of[1]=0 → false → break
        // so returns false (lit 2 is not false, clause not unit for lit 4)
        //
        // Actually let me think again. The clause is {x1, x2}. After reverse sort [4,2].
        // w[4]=1, w[2]=0. is_unit(4) checks clause 1: scan from i+1: l[i+1]=2.
        // literal_is_false(2, depth_of, moves): var=1, depth_of[1]=0 → false → not unit.
        // So is_unit(4) = false here.
        //
    ...
    fn test_solve_langford_sat() {
        use crate::sample_problems::langford;
        for n in [3, 4, 7] {
